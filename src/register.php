<?php
	...
	if(!isset($_POST['name']) || strlen($_POST['name']) > 255 || !preg_match('/^[a-zA-Z-áéíóúÁÉÍÓÚ ]+$/', $_POST['name'])) {
		$errors[] = 1;
	}
	...
	// 	$errors[] = 3;
	// }
	if(!isset($_POST['password']) || !preg_match('/^(?=.*[a-z])(?=.*[A-Z])(?=.*[0-4])(?=.*[\~?!@#\$%\^&\*])(?=.{8,})/', $_POST['password'])) {
		$errors[] = 4;
	}
		...
		if(isset($_POST['csrf_token']) && validateToken($_POST['csrf_token'])) {
			...
			if($C) {
				...
				if($res && $res->num_rows === 0) {
					...
					$id = sqlInsert($C, 'INSERT INTO users VALUES (NULL,?,?,?,?,?,0,0,?,2)','ssssss', $_POST['name'], $_POST['email'], $hash, $_POST['cargo'],$_POST['upp'],$created_at);
